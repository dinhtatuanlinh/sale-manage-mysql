...
let adminPage = async (req, res, next) => {
    ...
    if (req.user.username == "dinhtatuanlinh") {
        // req.user để lấy thông tin user
        let users;
        let options;
...
let adminEditSetting = async (req, res, next) => {
    ...
    if (req.user.username == "dinhtatuanlinh") {
        ...
        await database.Option.findOne({ where: { name: "avatar" } }).then(
            async (result) => {
                ...
                await database.Option.update(
                    ...
                    if (updateResult) {
                        req.flash(
                            "success",
                            "Update row avatar ở bảng option thành công",
                            false
                        );
                    } else {
                        req.flash(
                            "error",
                            "Update row avatar ở bảng option thất bại",
                            false
                        );
                    }
        ...
        user[0].team = req.body.status.team === 0 ? [] : req.body.team.split(",");
        ...
        await database.Option.findOne({ where: { name: "user" } }).then(
            async (result) => {
                if (result === null) {
                    ...
                    await database.Option.create(saveUser).then(
                        (saveResult) => {
                            if (saveResult) {
                                req.flash(
                                    "success",
                                    "Thêm mới row user ở bảng option thành công",
                                    false
                                );
                            } else {
                                req.flash(
                                    "error",
                                    "Thêm mới row user ở bảng option thất bại",
                                    false
                                );
                            }
                    ...
                    await database.Option.update(
                        ...
                        if (saveResult) {
                            req.flash(
                                "success",
                                "Update row user ở bảng option thành công",
                                false
                            );
                        } else {
                            req.flash(
                                "error",
                                "Update row user ở bảng option thất bại",
                                false
                            );
                        }
                        saveResult = JSON.stringify(saveResult);
        ...
        await database.Option.findOne({ where: { name: "customer" } }).then(
            async (result) => {
                if (result === null) {
                    ...
                    await database.Option.create(saveCustomer).then(
                        (saveResult) => {
                            if (saveResult) {
                                req.flash(
                                    "success",
                                    "Thêm mới row customer ở bảng option thành công",
                                    false
                                );
                            } else {
                                req.flash(
                                    "error",
                                    "Thêm mới row customer ở bảng option thất bại",
                                    false
                                );
                            }
                            saveResult = JSON.stringify(saveResult);
                    ...
                    await database.Option.update(
                        { value: customer, status: true },
                        { where: { name: "customer" } }
                    ).then((saveResult) => {
                        if (saveResult) {
                            req.flash(
                                "success",
                                "Update row customer ở bảng option thành công",
                                false
                            );
                        } else {
                            req.flash(
                                "error",
                                "Update row customer ở bảng option thất bại",
                                false
                            );
                        }
                        saveResult = JSON.stringify(saveResult);
